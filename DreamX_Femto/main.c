...
int main() {
    ...
    double *output_pmf_accum = NULL; // Will allocate after max value is known
    double *PMF = NULL; // Adjust size as needed

    processAndMerge(outputs,numAdders,rows,output_cols, 
                     accurateAdder, LV,  LV_size,configuration ,Probability_A_bits, Probability_B_bits, Probability_C_in, BW, &MSE,&MED, CPM_00, CPM_01, CPM_10, CPM_11,output_pmf_accum,PMF ) ;
